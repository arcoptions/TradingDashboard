...
def render(wb_obj, watchlist_symbols, sheet_headers, *args, **kwargs):
    ...
    if not df_raw_logs.empty:
        ...
        for idx, row in df_pending.iterrows():
            ...
            if matched_symbol and matched_symbol != "-":
        ...
        def render_bulk_table(df, tab_name):
            ...
            if b1.button("⚡ Stage Selected Rows", key=f"stg_{tab_name}", use_container_width=True):
                ...
                for _, s_row in selected_rows.iterrows():
                    is_beat_the_street = "beat the street" in str(s_row['Channel Source']).lower()
                    
                    if is_beat_the_street:
                        bulk_study_rows.append([
                            str(s_row['Timestamp']), 
                            str(s_row['Channel Source']), 
                            str(s_row.get('Extracted_Symbol', '-')), 
                            str(s_row['Raw Message Text']), 
                            datetime.today().strftime("%Y-%m-%d")
                        ])
                        status_updates.append({'range': f"D{s_row['_Row_ID']}", 'values': [["Staged to Study"]]})
                    else:
                        if "Duplicate" in str(s_row["Status"]): 
                            continue 
                        
                        t_sym, t_sec, t_exch = api.resolve_instrument(str(s_row.get('Extracted_Symbol', '-')))
                        ...
                        fill("Symbol / Asset", t_sym if t_sym else str(s_row.get('Extracted_Symbol', '-')))
                        fill("Trade Type (Eq/Option)", "Equity" if "EQ" in str(s_row.get('Exchange_Segment_Verified', '')) else "Option")
                        fill("Exchange", t_exch if t_exch else "NSE_EQ")
                        fill("Security ID", t_sec if t_sec else "")
